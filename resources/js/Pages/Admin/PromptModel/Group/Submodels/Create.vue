<template>  
    ...
    <HeaderSegment 
      :title="trans('Create Prompt Sub Model')" 
      :segments="segments" 
      :buttons="buttons"
      />
  <div class="container-fluid mt--6"> 
	...
	<div class="col-lg-5 mt-5">
		<strong>{{ trans('Create Prompt Sub Model') }}</strong>
...
const props = defineProps(['segments','buttons','prompt_group']);
...
const store = () => {
 form.post(route('admin.prompt-sub-models.store',props.prompt_group.id),{
    onSuccess: () => {
        form.reset()
        createToast(
        {
