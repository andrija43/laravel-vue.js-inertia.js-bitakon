    ...
    <div class="creator__grid-item mb-30">
      <div class="creator__grid-bg w-img">
        <Link :href="`/profile/${user.username}`">
          <img
            lazy="loading"
            v-lazy="user.cover_image ? user.cover_image : `/assets/img/creator/creator-bg.jpg`"
            alt="cover"
          />
        </Link>
      </div>
      <div class="creator__grid-content text-center">
        <div class="creator__grid-thumb">
          <Link :href="`/profile/${user.username}`">
            <img
              lazy="loading"
              v-lazy="user.avatar ? user.avatar : `https://ui-avatars.com/api/?name=${user.name}`"
              alt="avatar"
            />
          </Link>
        </div>
        ...
        <div class="creator__grid-social">
          <template v-if="userMeta?.socials">
            <a v-for="(social, key) in userMeta.socials" :key="key" :href="social" target="_blank">
              <i :class="`fa-brands fa-${key}`"></i>
            </a>
          </template>
        </div>
        <p v-if="userMeta?.bio">{{ textExcerpt(userMeta.bio, 40) }}</p>
      </div>
    </div>
  </div>
</template>
